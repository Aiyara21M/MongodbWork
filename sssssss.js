db.getCollection("patientorders").aggregate([
    {
        $match: {
            orderdate: {
                $gte: ISODate("2024-05-05T00:00:00.000+07:00"),
                $lte: ISODate("2024-05-05T23:59:59.999+07:00")
            },
        }
    },
    ...
    },
    {
        $group:{
           ...
           { $lookup: { from: 'wards', localField: 'badmove.warduid', foreignField: '_id', as: 'ward' } },
           { $unwind: { path: '$ward', preserveNullAndEmptyArrays: true } },
  {
    $group: {
        _id: "$_id",
