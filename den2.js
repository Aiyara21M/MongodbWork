db.getCollection("patientbills").aggregate([
 {
     $match: {
         billdate:{
	      $gte: ISODate("2024-04-18T00:00:00.000+07:00"),
          $lte: ISODate("2024-04-18T23:59:59.999+07:00")
        },
         "isrefund" : false,
         "iscancelled" : false
    }
     ...
     $match: {
       ...
       {$group:{
           ...
           comments:{
              $switch: {
            branches: [
          { case: { $eq: ["$comments", "3"] }, then: 3 },
          { case: { $eq: ["$comments", "2"] }, then: 2 },
          { case: { $eq: ["$comments", "1"] }, then: 1 },
          { case: { $eq: ["$comments", null] }, then: 0 },
          { case: { $eq: ["$comments", ""] }, then: 0 },
        ],
        default: "$comments" 
      }
    },
